...
#define _CosSinPhi          _SkyParam.zw
	...
	float4 GetColorWithRotation(float3 dir, float exposure, float2 cos_sin)
	{
		...
		float3 skyColor2 = SAMPLE_TEXTURECUBE_LOD(_Cubemap2, sampler_Cubemap2, dir, 0).rgb * _Intensity * exposure;
		skyColor = ClampToFloat16Max(lerp(skyColor, skyColor2, _TransitionLerp));
